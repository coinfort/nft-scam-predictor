...
```bash
nft-scam-predictor$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
nft-scam-predictor$ sam local start-api
nft-scam-predictor$ curl http://localhost:3000/classify_digit
```
...
```bash
nft-scam-predictor$ sam logs -n InferenceFunction --stack-name nft-scam-predictor --tail
```
...
```bash
aws cloudformation delete-stack --stack-name nft-scam-predictor
```
